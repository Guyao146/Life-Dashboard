const $=s=>document.querySelector(s), c=$('#clock'), modal=$('#connect-modal'), toast=$('#toast');
    ...
    const widgetKey='life-hub-widgets',layoutKey='life-hub-layout',navKey='life-hub-hidden-nav',widgetNames={home:'家庭环境',delivery:'外卖配送',agenda:'今日日程',quick:'快捷控制',devices:'HA 区域与场景',habit:'本周习惯',energy:'能量使用'},navNames={home:'我的家',schedule:'日程',orders:'消费与订单',health:'健康'};
    ...
    function applyView(){const hidden=hiddenWidgets();applyLayout();applyNavVisibility();document.querySelectorAll('.grid>[data-widget-id]').forEach(card=>{const inView=activeView==='overview'||card.dataset.views.split(' ').includes(activeView);card.classList.toggle('is-hidden',!inView||hidden.has(card.dataset.widgetId))});$('.grid').classList.toggle('is-hidden',activeView==='settings');$('.settings-page').classList.toggle('is-active',activeView==='settings')}
    ...
    async function checkForUpdate(){const status=$('#version-status'),button=$('#check-update');if(!status||!button)return;button.disabled=true;status.textContent='正在检查 GitHub…';try{let remoteText='';for(const source of versionSources){const response=await fetch(`${source}?t=${Date.now()}`,{cache:'no-store'});if(response.ok){remoteText=await response.text();break}}if(!remoteText)throw new Error('无法读取远程版本');const match=remoteText.match(/version:\s*['"]([^'"]+)['"]/);const remoteVersion=match?.[1];if(!remoteVersion)throw new Error('远程版本格式无法识别');if(remoteVersion!==release?.version){status.textContent=`发现新版本 v${remoteVersion}`;notice(`发现新版本 v${remoteVersion}，点击 GitHub 查看`)}else{status.textContent='已是最新版本';notice('当前已经是最新版本')}}catch(error){status.textContent='检查失败，请稍后重试';notice('暂时无法检查 GitHub 更新')}finally{button.disabled=false}}
    function setView(view){if(view===activeView)return;const before=new Map(visibleCards().map(card=>[card,card.getBoundingClientRect()]));activeView=view;document.querySelectorAll('[data-view]').forEach(item=>{const isActive=item.dataset.view===view;item.classList.toggle('active',isActive);if(item.closest('.nav,.mobile-nav'))item.setAttribute('aria-current',isActive?'page':'false')});renderViewIntro(view);applyView();if(view==='settings'){renderWidgetSettings();renderNavSettings();syncProfileSettings();requestAnimationFrame(animateSettingsView)}else animateViewChange(before);window.scrollTo({top:0,behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth'})}
    ...
    $('#settings-reset').onclick=()=>{localStorage.removeItem(widgetKey);localStorage.removeItem(layoutKey);localStorage.removeItem(navKey);localStorage.removeItem('life-hub-quick');localStorage.removeItem(environmentSlotsKey);renderWidgetSettings();renderNavSettings();renderEnvironmentSlots(latestStates);setView('overview');notice('布局、小组件、导航和环境数据位已恢复默认')};
    const grid=$('.grid');let draggedCard=null;const reorderAnimations=new WeakMap();
    setupWidgetMenus();
    document.addEventListener('click',()=>closeWidgetMenus());document.addEventListener('keydown',e=>{if(e.key!=='Escape')return;closeWidgetMenus();closeEnvironmentPicker();$('#picker-modal').classList.remove('show');$('#device-modal').classList.remove('show')});window.addEventListener('resize',()=>{applyLayout();closeWidgetMenus()});
    ...
    document.querySelectorAll('.more:not(#refresh-home):not(#quick-picker):not(#close-detail):not(#close-environment-picker)').forEach(button=>button.onclick=()=>notice('该数据源尚未接入，可继续扩展'));
