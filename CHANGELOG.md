...
- 登录页在发现已有单点登录会话时，仅显示对应的身份继续入口；切换其他账号后正确恢复账号密码与 OAuth 登录方式。
- 优化窄屏登录页的间距和按钮尺寸，提升手机端登录体验。
...
- 加载页动画改为品牌 Logo 外圈环绕一条 Material Design 风格的不定长弧线：弧线一边整体旋转、一边伸长收短，Logo 本体缓慢呼吸缩放。
- 移除首字母头像与旧的三点跳动进度指示，加载页只保留 Logo 动画、问候语与阶段说明三层信息。
- 弧线在夜间主题使用更亮的品牌绿；系统开启「减少动态效果」时弧线定长慢转，Logo 不再缩放。

## [1.0.5] - 2026-08-31

### 变更

- 重做加载页身份问候的样式：去掉边框胶囊，改为转圈光环包住首字母头像，问候语「晚上好，账号名」作为主标题，原阶段标题退为下方小字说明。
- 未识别身份时加载页保持原有 34px 转圈布局，识别到身份后光环平滑放大到 64px。

## [1.0.4] - 2026-08-31

### 新增

- 加载页顶部新增身份问候胶囊：显示首字母头像与「早上好，欢迎回来 / 账号名」，让静默单点登录进入看板的过程不再毫无反馈。
- 问候名称按 Authentik 身份 → 本地账号 → 设置中的称呼依次回退；管理员私密配置加载完成后会自动刷新为真实账号名。
- 问候胶囊适配夜间主题，并在系统开启「减少动态效果」时关闭入场动画。
